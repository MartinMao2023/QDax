...
class DCRLEmitter(Emitter):
    ...
    def __init__(
        ...
        self._estimator_network = estimator_network
        ...
        self._estimator_optimizer = optax.adam(
            learning_rate=self._config.critic_learning_rate
        )
        ...
        # define the loss function for sof predictor
        @jax.jit
        def estimator_loss(
            estimator_params: Params,
            target_estimator_params: Params,
            target_actor_params: Params,
            ...
            delta_y = (
                ...
                )[:, :, None, None] # batch x D x 1 x 1
            
            estimator_outcomes = estimator_network.apply(
                estimator_params, 
                transitions.obs, 
                transitions.actions, 
            ...
            successor_outcomes = estimator_network.apply(
                target_estimator_params, 
                transitions.next_obs, 
                successor_actions, 
                transitions.desc_prime
                ) 

            # flatten to zero if terminate
            successor_outcomes = jnp.where(transitions.dones[:, jnp.newaxis],
                                           jnp.zeros_like(successor_outcomes),
                                           successor_outcomes)[:, :, None, :] # W_q^T: batch x D x 1 x d

            losses = jnp.squeeze(
                (jnp.swapaxes(estimator_outcomes, -1, -2) @ S @ estimator_outcomes)
                - 2 * delta_y * (V @ estimator_outcomes) 
                - 2 * successor_outcomes @ B @ estimator_outcomes
                , axis=(-1, -2)) * transitions.truncations[:, None] # N x D 
            
            # one can add a (1 x D) weighting vector here. So:
            # losses *= weighting

            return jnp.mean(losses)
    
        self._estimator_loss_fn = estimator_loss
    ...
    def _predict(self, 
                 ...
                 descs_2: Descriptor, 
                 t: jnp.ndarray,
                 emitter_state: DCRLEmitterState) -> jnp.array:
        """
        Args:
            ???
        Returns:
